...
def delete_hizmet(hizmet_id: int, db: Session = Depends(get_db)):
    ...
    tarife_kullanim = db.query(PriceListItem).filter(
        PriceListItem.HizmetKodu == hizmet.Kod
    ).count()
    
    if tarife_kullanim > 0:
        raise HTTPException(
            status_code=400,
            detail=f"Bu hizmet {tarife_kullanim} adet tarife kaleminde kullanılmaktadır. Silme işlemi yapılamaz."
        )
    
    # Eğer işlem görmemişse sil
    db.delete(hizmet)
    db.commit()
