...
def seed_work_orders(db: Session):
    ...
    wo2 = WorkOrder(
        wo_number=f"WO-{now.year}-{len(work_orders)+1:05d}",
        cari_id=cari_list[1].Id,
        cari_code=cari_list[1].CariKod,
        cari_title=cari_list[1].Unvan,
        type=WorkOrderType.HIZMET,
        service_id=hizmet_list[1].Id if len(hizmet_list) > 1 else None,
        service_code=hizmet_list[1].Kod if len(hizmet_list) > 1 else None,
        ...
        planned_start=now + timedelta(days=1, hours=14),
        planned_end=now + timedelta(days=1, hours=20),
        Decimal("12500.00"),
        "TRY"
    )
    ...
    wo3 = WorkOrder(
        wo_number=f"WO-{now.year}-{len(work_orders)+1:05d}",
        cari_id=cari_list[2].Id,
        cari_code=cari_list[2].CariKod,
        cari_title=cari_list[2].Unvan,
        type=WorkOrderType.HIZMET,
    ...
    wo6 = WorkOrder(
        ...
        planned_end=now + timedelta(days=3, hours=4),
        Decimal("0"),
        "TRY"
    )
    ...
    for i in range(4):
        ...
        wo = WorkOrder(
            ...
            priority=WorkOrderPriority.MEDIUM,
            status=statuses[i],
            planned_start=now + timedelta(days=i+1),
            planned_end=now + timedelta(days=i+1, hours=4),
            Decimal(f"{1000 * (i+1)}.00"),
            "TRY"
        )
